...
## PCMCI plus
def tensor_to_pcmciplus_res(sample, c_test, max_tau):
    dataframe = pp.DataFrame(
        sample.detach().numpy().astype(float),
        datatime=np.arange(len(sample)),
        var_names=np.arange(sample.shape[1]),
    ...
    )                                                                                   # and in CP model no cont. edges are inferred
    return q_matrix

@timing
def pcmciplus_ci(ds, c_test, max_lags=3):
    out = []
    for sample in ds:
        q_matrix = tensor_to_pcmciplus_res(sample, c_test, max_tau=max_lags)
        out.append(np.swapaxes(np.flip(q_matrix[:, :, 1:], 2), 0, 1))
    out = np.stack(out, axis=0)
    return out

def pcmciplus_run_full_ds(dl, nonlinear=False):
    if nonlinear:
        c_test = GPDC()
    else:
        c_test = ParCorr()
    ...
    lab = []
    for x, y in dl:
        out.append(pcmciplus_ci(x, c_test, max_lags=y.shape[3])[0])
        lab.append(y)
    out = torch.Tensor(np.concatenate(out, axis=0))
    lab = torch.concat(lab, axis=0)
    return out, lab
